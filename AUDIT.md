...
| Frontend runtime | `npm install` succeeds, but **`sistemnya/build/bundle-legacy.mjs` (the legacy bundler) is MISSING** from this checkout (the whole `build/` dir is absent), and `vite.config.js` invokes it at `buildStart`. So `npm run build`/`dev` and even `vite build` **all fail**; only a stale generated `src/main.jsx` (2026-06-02) is present. | **⚠ Phase 5 (frontend) is BLOCKED** until the owner restores `sistemnya/build/` from the real repo (`rafiarrantisi/new-simulator-anamnesis`). The backend is fully buildable/testable; this gap is frontend-only. |
