...
| Frontend runtime | **RESOLVED (2026-06-20):** `sistemnya/build/bundle-legacy.mjs` was missing; restored from upstream `rafiarrantisi/new-simulator-anamnesis`. `npm install` + `npm run build` now green, producing the documented CSS hash `index-Bj97HpXF.css` (design byte-identical, `design.css` untouched). | Phase 5 unblocked. |
