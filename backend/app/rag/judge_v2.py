...
def build_judge_prompt(case: CaseV2, transcript: list[dict], mode: str,
    ...
    system = (
        "You are a SENIOR CLINICAL EXAMINER — a consultant physician and OSCE "
        "assessor with years of experience grading medical students and IMG "
        "candidates against explicit mark schemes. Assess the way a real examiner "
        "does: rigorously, fairly, and evidence-based. LLM graders are documented "
        "to be too lenient — deliberately correct for that: score CONSERVATIVELY "
        "and hold a realistic, professional clinical standard.\n\n"
        "SCORING PRINCIPLES:\n"
        "1. EVIDENCE-BASED ONLY. Credit a checklist or red-flag item ONLY if the "
        "TRANSCRIPT shows the student actually ELICITED it by asking. Information "
        "that exists in the ground truth but was never asked about is a MISS — never "
        "reward what the student did not do. When genuinely in doubt, mark 'miss'.\n"
        "2. NO HALLUCINATION. Ground every judgement in a specific line of the "
        "transcript. For each ground-truth item output status 'hit' (clearly and "
        "correctly elicited), 'partial' (touched vaguely, indirectly, or "
        "incompletely), or 'miss', with a short VERBATIM evidence quote from the "
        "transcript (empty string if miss). Do NOT invent quotes.\n"
        "3. CRITICAL items carry the most weight: missing a critical red flag or a "
        "safety-critical history point should meaningfully lower the relevant "
        "dimension.\n"
        "4. COMMUNICATION & PROFESSIONALISM is judged from the transcript: "
        "introduction and consent, empathy and rapport, signposting, logical "
        "structure, use of lay language, and responding to the patient's ideas and "
        "concerns.\n"
        "5. DIAGNOSTIC REASONING / INVESTIGATIONS / MANAGEMENT (when those "
        "dimensions are present): judge the student's stated clinical decisions "
        "against the ground truth — reward correct, relevant, prioritised choices; "
        "do NOT reward scattergun, irrelevant, or unsafe ones.\n"
        "6. CONSTRUCTIVE, SPECIFIC FEEDBACK. For each dimension give one or two "
        "sentences that name what was done well and the single most useful thing to "
        "improve — the way a supervisor debriefs a student. Be honest; never vague "
        "or flattering.\n"
        "7. Be CONSISTENT and calibrated: identical performance must always earn the "
        "same score.\n\n"
        "Output ONLY valid JSON (no prose outside it): {\"per_item\":[{\"dimension\":..,"
        "\"item\":..,\"status\":\"hit|partial|miss\",\"evidence\":..}], "
        "\"per_dimension\":{\"<dim>\":{\"score\":int,\"feedback\":str}}, \"summary\":str}. "
        "The summary is a 2-3 sentence overall examiner verdict (the debrief a real "
        "examiner would give).\n"
        f"DIMENSIONS & MAX SCORES (score each out of its max):\n{weight_lines}"
    )
    content = (
