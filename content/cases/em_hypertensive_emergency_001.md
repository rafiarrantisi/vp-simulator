...
languages: [en]
source_refs: [ "PNPK Hipertensi Dewasa (KMK 4634/2021) — https://jdih.kemkes.go.id/documents/keputusan-menteri-kesehatan-nomor-hk0107menkes46342021" ]
authoring: { drafted_by: ai_v1, model: "deepseek/deepseek-v4-flash", reviewed_by: null, reviewed_at: null, review_notes: null }
chief_complaint: "I have a very bad headache and my vision is blurry."
anamnesis_checklist:
  hpi_socrates:
    - { item: "Onset (when did the headache start?)", critical: false }
    - { item: "Location (where exactly is the headache?)", critical: false }
    - { item: "Quality (what does the headache feel like?)", critical: false }
    - { item: "Severity (how bad is the pain on a scale of 0-10?)", critical: false }
    ...
    - { item: "Associated symptoms (blurred vision, nausea, chest pain, shortness of breath?)", critical: true }
  associated_symptoms:
    - { item: "Nausea or vomiting", critical: false }
    - { item: "Chest pain or tightness", critical: true }
    - { item: "Shortness of breath", critical: true }
    - { item: "Numbness or weakness in arms or legs", critical: true }
    - { item: "Difficulty speaking", critical: true }
  pmh:
    - { item: "History of high blood pressure", critical: true }
    - { item: "History of diabetes", critical: false }
    - { item: "History of kidney disease", critical: false }
    - { item: "History of stroke or heart attack", critical: true }
  medications:
    - { item: "Current blood pressure medications", critical: true }
    - { item: "Medication adherence (do you take them regularly?)", critical: true }
    - { item: "Any other medications or supplements", critical: false }
  family_social:
    - { item: "Family history of high blood pressure or stroke", critical: false }
...
red_flags:
  - { item: "Severe headache with blurred vision", critical: true }
  - { item: "Blood pressure >180/120 mmHg", critical: true }
  - { item: "Chest pain or shortness of breath", critical: true }
  - { item: "Neurological symptoms (weakness, numbness, speech difficulty)", critical: true }
expected_ddx:
  working_diagnosis: "Hypertensive emergency"
  differentials: [ "Hypertensive urgency", "Acute ischemic stroke", "Migraine with aura" ]
investigations:
  appropriate:
    - { name: "Blood pressure measurement", expected: ">180/120 mmHg" }
    - { name: "Fundoscopy", expected: "Papilledema or retinal hemorrhages" }
    - { name: "ECG", expected: "Left ventricular hypertrophy or ischemia" }
    - { name: "Serum creatinine", expected: "Elevated (if renal involvement)" }
    - { name: "Urinalysis", expected: "Proteinuria or hematuria" }
  inappropriate: [ "Carotid ultrasound" ]
physical_exam_findings:
  general: "Anxious, in moderate distress, holding head with both hands"
  vitals: { bp: "210/130", hr: 100, rr: 18, temp: 36.8, spo2: 98 }
management:
  pharmacological:
